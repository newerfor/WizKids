...
import com.example.wizkids.presentation.addNewOrChangeChild.constant.AddNewOrChangeChildViewConstant.ADD_NEW_OR_CHANGE_CHILD_BUTTON_WEIGHT
        ...
        childLearningStages.value.forEachIndexed { index, string ->
            Column(Modifier.padding(vertical = ADD_NEW_OR_CHANGE_LEARNING_STAGES_COLUMN_PADDING.dp)){
                Row(Modifier.fillMaxSize(), verticalAlignment = Alignment.CenterVertically){
                    textFont.GrayText("${index+ADD_NEW_OR_CHANGE_CHILD_DEFAULT_VALUE_INDEX_IN_LIST}.",Modifier.padding(end = ADD_NEW_OR_CHANGE_LEARNING_STAGES_TEXT_PADDING.dp))
                    TextFieldVisible().TextOutlineField(string, textFont,
                        stringResource(R.string.development_stage_hint), trailingIcon = childWorkStageError.value) { newText ->
                    ...
                    IconButton(
                        onClick = {
                            childLearningStages.value = childLearningStages.value.toMutableList().apply {
                                removeAt(index)
                            }
                        },
                    ) {
        ...
        ButtonView().ButtonVisibleRow(
            mapOf(
                stringResource(R.string.add_button) to {
                    childLearningStages.value = childLearningStages.value + ADD_NEW_OR_CHANGE_CHILD_DEFAULT_VALUE_WORK_STAGE
                },
