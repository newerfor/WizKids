package com.example.wizkids.presentation.addNewOrChangeChild.constant

object AddNewOrChangeChildLogicConstant {
    const val  ADD_NEW_OR_CHANGE_CHILD_DEFAULT_VALUE_NAME = ""
    const val  ADD_NEW_OR_CHANGE_CHILD_DEFAULT_VALUE_ADDITIONAL_INFO = ""
    const val  ADD_NEW_OR_CHANGE_CHILD_DEFAULT_VALUE_DAY_OF_BIRTH = ""
    const val  ADD_NEW_OR_CHANGE_CHILD_DEFAULT_VALUE_PRICE = 0
    const val  ADD_NEW_OR_CHANGE_CHILD_DEFAULT_VALUE_BALANCE = 0
    const val  ADD_NEW_OR_CHANGE_CHILD_DEFAULT_VALUE_IMAGE_PATCH = ""
    const val  ADD_NEW_OR_CHANGE_CHILD_DEFAULT_VALUE_WORK_STAGE = ""
    const val  ADD_NEW_OR_CHANGE_CHILD_DEFAULT_VALUE_INDEX_IN_LIST = 1
    const val  ADD_NEW_OR_CHANGE_CHILD_DEFAULT_INDEX_FIRST_NAME = 0
    const val  ADD_NEW_OR_CHANGE_CHILD_DEFAULT_INDEX_MIDDLE_NAME = 2
    const val  ADD_NEW_OR_CHANGE_CHILD_DEFAULT_INDEX_LAST_NAME = 1

}